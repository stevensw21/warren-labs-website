...
export default function ScubaMapMaker() {
  ...
  const routeStrokeStyle = { "--route-hit-height": `${14 / zoom}px`, "--route-hit-offset": `${-6 / zoom}px`, "--route-label-border": `${1 / zoom}px`, "--route-label-radius": `${3 / zoom}px`, "--route-label-font": `${10 / zoom}px`, "--route-label-padding-y": `${2 / zoom}px`, "--route-label-padding-x": `${4 / zoom}px` } as CSSProperties;
      ...
      <section className="workspace">
        ...
        <div className={`map-stage ${tool === "select" ? "pan-ready" : ""}${printArea ? " has-print-area" : ""}`} style={printCropStyle} ref={stageRef} tabIndex={0} aria-label="Map navigation surface. Drag blank map space to pan; scroll to zoom; use arrow keys to move around." onPointerDown={onStagePointerDown} onPointerMove={onStagePointerMove} onPointerUp={onStagePointerUp} onPointerCancel={onStagePointerUp} onWheel={onStageWheel}>
          <div className="map-canvas" onPointerDown={onStagePointerDown} style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }}>
            ...
            {shorelinePath && <svg className={`shoreline-path${exportClass(exportSettings.shoreline)}`} viewBox={`0 0 ${CANVAS.width} ${CANVAS.height}`} aria-hidden="true"><path className="shoreline-base" d={shorelinePath} fill="none" stroke="#d2c093" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke"/><path className="shoreline-edge" d={shorelinePath} fill="none" stroke="#867750" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke"/></svg>}
            ...
            {project.vectors.map((vector) => { const line = geometry(project, vector); if (!line) return null; return <div className="vector-group" key={vector.id}><button className={`vector-line ${selectedVectorId === vector.id ? "selected-vector" : ""}${exportClass(exportSettings.routes)}`} aria-label={`Select route from ${line.start.label} to ${line.end.label}`} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); selectVector(vector); }} style={{ ...routeStrokeStyle, left: line.start.x, top: line.start.y, width: line.pixels, transform: `rotate(${Math.atan2(line.dy, line.dx)}rad)` }}><svg viewBox={`0 0 ${Math.max(line.pixels, 1)} 14`} aria-hidden="true"><line x1="2" y1="7" x2={Math.max(2, line.pixels - 10)} y2="7"/><path d={`M${Math.max(2, line.pixels - 10)} 2 L${Math.max(2, line.pixels - 1)} 7 L${Math.max(2, line.pixels - 10)} 12 Z`}/></svg></button><button className={`vector-label${exportClass(exportSettings.routeLabels)}`} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); selectVector(vector); }} style={{ ...routeStrokeStyle, left: line.start.x + line.dx / 2, top: line.start.y + line.dy / 2 }}>{Math.round(vector.bearing)}° · {formatNumber(vector.distance)} {vector.unit}</button></div>; })}
